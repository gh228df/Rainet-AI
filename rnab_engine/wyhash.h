...
unsigned SIMD_NAME(sprp)(unsigned long long n, unsigned long long a) {
...
unsigned SIMD_NAME(is_prime)(unsigned long long n) {
    if (n<2||!(n&1)) return 0;
    if (n<4) return 1;
    if (!SIMD_NAME(sprp)(n,2)) return 0;
    if (n<2047) return 1;
    if (!SIMD_NAME(sprp)(n,3)) return 0;
    if (!SIMD_NAME(sprp)(n,5)) return 0;
    if (!SIMD_NAME(sprp)(n,7)) return 0;
    if (!SIMD_NAME(sprp)(n,11)) return 0;
    if (!SIMD_NAME(sprp)(n,13)) return 0;
    if (!SIMD_NAME(sprp)(n,17)) return 0;
    if (!SIMD_NAME(sprp)(n,19)) return 0;
    if (!SIMD_NAME(sprp)(n,23)) return 0;
    if (!SIMD_NAME(sprp)(n,29)) return 0;
    if (!SIMD_NAME(sprp)(n,31)) return 0;
    if (!SIMD_NAME(sprp)(n,37)) return 0;
    return 1;
}
...
static inline void make_secret(uint64_t seed, uint64_t *secret){
  ...
  for(size_t i=0;i<4;i++){
    ...
    do{
      ...
      if(ok&&!SIMD_NAME(is_prime)(secret[i]))	ok=0;
    }while(!ok);
  }
}
