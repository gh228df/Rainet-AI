...
SIMD_NAME(minimax_main)(const int depth, int alpha, int beta, const bool player, field_t *position)
{
    ...
    if (player)
    {
        ...
        boost::unordered_flat_map<field_t, ttentry_t, field_t> newcache[MAX_DEPTH];
        ...
        for (int current_depth = 2; current_depth <= max_depth; current_depth += 2)
        {
            ...
            for (int i = 0; i < all_moves.moves_count; ++i)
            {
                ...
                if (elapsed_time > max_search_time)
                {
                    ...
                    return (minimax_main_result_t){.best_field = best_field, .evaluation = iteration_alpha, .has_timed_out = true};
                }
            }
        ...
        for (int current_depth = 2; current_depth <= max_depth; current_depth += 2)
        {
            ...
            for (int i = 0; i < all_moves.moves_count; ++i)
            {
                ...
                if (elapsed_time > max_search_time)
                {
                    ...
                    return (minimax_main_result_t){.best_field = best_field, .evaluation = iteration_beta, .has_timed_out = true};
                    ;
                }
            }
