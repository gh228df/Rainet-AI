...
void *ai_move_thread(void *args)
{
    ...
    int64_t cur_time = (stop.tv_sec * 1000000000 + stop.tv_nsec - start.tv_sec * 1000000000 - start.tv_nsec) / 1000000;

    if (cur_time < 500 && adaptive_ai_level < 20)
    {
        adaptive_ai_level += 2;
    }
    else if (cur_time > 10000 && adaptive_ai_level > ai_level)
    {
        adaptive_ai_level -= 2;
    }
...
void place_card(struct button_t *self)
{
    ...
    {
        pos = field_t(indexes[rand() % 70], player_field_id);
