        ...
        {                                                                                                                                                              \
            ...
            {                                                                                                                                                          \
                ...
                {                                                                                                                                                      \
                    ...
                    int reschild = (depth > 0) ? SIMD_NAME(minimax)(depth, alpha, beta, false, temp_field, cache) : temp_field.evaluate();                                        \
                    BRANCH_EXIT_MAX();                                                                                                                                 \
                ...
                {                                                                                                                                                      \
                    ...
                    {                                                                                                                                                  \
                        ...
                        reschild = (depth > 0) ? SIMD_NAME(minimax)(depth, alpha, beta, false, temp_field, cache) : temp_field.evaluate();                                        \
                        BRANCH_EXIT_MAX();                                                                                                                             \
                    }                                                                                                                                                  \
                    else                                                                                                                                               \
                    {                                                                                                                                                  \
                        ...
                        if (depth > 0)                                                                                                                                 \
                        {                                                                                                                                              \
                            reschild = SIMD_NAME(minimax)(depth, alpha, alpha + 1, false, temp_field, cache);                                                                     \
                            if (reschild > alpha && beta > alpha + 1)                                                                                                  \
                                reschild = SIMD_NAME(minimax)(depth, alpha, beta, false, temp_field, cache);                                                                      \
                        }                                                                                                                                              \
                        else                                                                                                                                           \
        ...
        {                                                                                                                                                              \
            ...
            {                                                                                                                                                          \
                ...
                {                                                                                                                                                      \
                    ...
                    int reschild = (depth > 0) ? SIMD_NAME(minimax)(depth, alpha, beta, true, temp_field, cache) : temp_field.evaluate();                                         \
                    BRANCH_EXIT_MIN();                                                                                                                                 \
                ...
                {                                                                                                                                                      \
                    ...
                    {                                                                                                                                                  \
                        ...
                        reschild = (depth > 0) ? SIMD_NAME(minimax)(depth, alpha, beta, true, temp_field, cache) : temp_field.evaluate();                                         \
                        BRANCH_EXIT_MIN();                                                                                                                             \
                    }                                                                                                                                                  \
                    else                                                                                                                                               \
                    {                                                                                                                                                  \
                        ...
                        if (depth > 0)                                                                                                                                 \
                        {                                                                                                                                              \
                            reschild = SIMD_NAME(minimax)(depth, beta - 1, beta, true, temp_field, cache);                                                                        \
                            if (reschild < beta && alpha < beta - 1)                                                                                                   \
                                reschild = SIMD_NAME(minimax)(depth, alpha, beta, true, temp_field, cache);                                                                       \
                        }                                                                                                                                              \
                        else                                                                                                                                           \
                            reschild = temp_field.evaluate();                                                                                                          \
                                                                                                                                                                       \
...
int64_t SIMD_NAME(rec_counter) = 0;

int SIMD_NAME(minimax)(int depth, int alpha, int beta, const bool player, const field_t &position, boost::unordered_flat_map<field_t, ttentry_t, field_t> *cache)
{
#ifdef BRANCH_DEBUG
    ...
    if (player)
    {
        ...
        if (position.is_firewall_available_fir && depth > SIMD_NAME(cur_search_depth) - COSTLY_POWERUPS_LOOKAHEAD)
        {
            uint64_t temp = fir_link_mask & 16717361816799281127ULL;
        ...
        if (position.is_swap_available_fir && depth > SIMD_NAME(cur_search_depth) - COSTLY_POWERUPS_LOOKAHEAD)
        {
            uint64_t link_mask = fir_link_mask;
    ...
    else
    {
        ...
        if (position.is_firewall_available_sec && depth > SIMD_NAME(cur_search_depth) - COSTLY_POWERUPS_LOOKAHEAD)
        {
            uint64_t temp = sec_link_mask & 16717361816799281127ULL;
        ...
        if (position.is_swap_available_sec && depth > SIMD_NAME(cur_search_depth) - COSTLY_POWERUPS_LOOKAHEAD)
        {
            uint64_t link_mask = sec_link_mask;
...
SIMD_NAME(minimax_main)(const int depth, int alpha, int beta, const bool player, field_t &position)
{
    ...
    if (player)
    {
        ...
        for (int i = 0; i < all_moves.moves_count; ++i)
        {
            ...
            if (is_first_move)
            {
                childres = SIMD_NAME(minimax)(depth - 1, alpha, beta, false, all_moves.moves[i], newcache);
            ...
            {
                childres = SIMD_NAME(minimax)(depth - 1, alpha, alpha + 1, false, all_moves.moves[i], newcache);
                clock_gettime(CLOCK_MONOTONIC, &stop);
                if (childres > alpha)
                {
    ...
    else
    {
        ...
        for (int i = 0; i < all_moves.moves_count; ++i)
        {
            ...
            if (is_first_move)
            {
                childres = SIMD_NAME(minimax)(depth - 1, alpha, beta, true, all_moves.moves[i], newcache);
                clock_gettime(CLOCK_MONOTONIC, &stop);
            ...
            {
                childres = SIMD_NAME(minimax)(depth - 1, beta - 1, beta, true, all_moves.moves[i], newcache);
                clock_gettime(CLOCK_MONOTONIC, &stop);
                if (childres < beta)
                {
...
minimax_main_result_t
SIMD_NAME(minimax_iteration_main)(const int depth, int alpha, int beta, const bool player, field_t &position)
{
    SIMD_NAME(cur_search_depth) = depth;
    ...
    if (player)
    {
        ...
        for (int current_depth = 2; current_depth <= depth; current_depth += 2)
        {
            ...
            for (int i = 0; i < all_moves.moves_count; ++i)
            {
                ...
                int childres = SIMD_NAME(minimax)(current_depth - 1, iteration_alpha, iteration_alpha + 1, false, all_moves.moves[move_idx], newcache);
                if (childres > iteration_alpha)
                    childres = SIMD_NAME(minimax)(current_depth - 1, iteration_alpha, beta, false, all_moves.moves[move_idx], newcache);
            ...
            if (iteration_alpha == prev_alpha - 56)
            {
                ...
                for (int i = 0; i < all_moves.moves_count; ++i)
                {
                    ...
                    {
                        childres = SIMD_NAME(minimax)(current_depth - 1, iteration_alpha, iteration_alpha + 1, false, all_moves.moves[move_idx], newcache);
                        if (childres > iteration_alpha)
                            childres = SIMD_NAME(minimax)(current_depth - 1, iteration_alpha, beta, false, all_moves.moves[move_idx], newcache);
                    }
    ...
    else
    {
        ...
        for (int current_depth = 2; current_depth <= depth; current_depth += 2)
        {
            ...
            for (int i = 0; i < all_moves.moves_count; ++i)
            {
                ...
                int childres = SIMD_NAME(minimax)(current_depth - 1, iteration_beta - 1, iteration_beta, true, all_moves.moves[move_idx], newcache);
                if (childres < iteration_beta)
                    childres = SIMD_NAME(minimax)(current_depth - 1, alpha, iteration_beta, true, all_moves.moves[move_idx], newcache);
            ...
            if (iteration_beta == prev_beta + 56)
            {
                ...
                for (int i = 0; i < all_moves.moves_count; ++i)
                {
                    ...
                    {
                        childres = SIMD_NAME(minimax)(current_depth - 1, iteration_beta - 1, iteration_beta, true, all_moves.moves[move_idx], newcache);
                        if (childres < iteration_beta)
                            childres = SIMD_NAME(minimax)(current_depth - 1, alpha, iteration_beta, true, all_moves.moves[move_idx], newcache);
                    }
